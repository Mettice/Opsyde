// Complete Enhanced Agent Editor with Framework/LLM Separation
...
const EnhancedAgentEditor = ({ 
  ...
  // Get available LLMs based on selected framework
  const getAvailableLLMs = () => {
    const compatibleLLMs = FRAMEWORK_LLM_COMPATIBILITY[formData.framework] || [];
    return AVAILABLE_LLM_PROVIDERS.filter(llm => compatibleLLMs.includes(llm.value));
  };
  ...
  const renderLLMSelector = () => {
    ...
    return (
      <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
        <h4 className="text-sm font-medium text-green-800 mb-4">🤖 LLM Configuration</h4>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {availableLLMs.map(llm => (
              <label 
                key={llm.value}
                className={`flex items-center p-3 border rounded-md cursor-pointer transition-colors hover:bg-gray-50 ${
                  (formData.llm?.provider || formData.llmProvider) === llm.value 
              ...
              >
                <input
                  ...
                  name="llmProvider"
                  value={llm.value}
                  checked={(formData.llm?.provider || formData.llmProvider) === llm.value}
